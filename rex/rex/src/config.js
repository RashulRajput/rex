...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\rex-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'rex-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/rex-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'rex-config');
    } else {
        // Linux and others: ~/.config/rex-config
        configDir = path.join(os.homedir(), '.config', 'rex-config');
    }
